...
class WikiTime(Job):
    ...
    def map_reader(input_fd, size, url, params):
        ...
        # print 'hello'
        # data = StringIO(fd.read())
        # print 'hello2'
        # df = pd.read_csv(data,compression='gzip',sep=' ',names=['project_code','file','views','bytes'])
        
        
        filtered = df[df['project_code'].isin(['en','fr'])]
        #filtered = df[df['project_code'].isin(['en','ja','fr','de','nl','zh','ne','he','is','uz','it','af','ar'])]
...
if __name__ == "__main__":
    from wiki_log_2 import WikiTime
    import numpy as np
    import pandas as pd
